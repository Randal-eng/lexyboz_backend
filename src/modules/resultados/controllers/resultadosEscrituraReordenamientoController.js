...
const registrarResultado = async (req, res) => {
    try {
        const { usuario_id, tipo_subtipo, tiempo_inicio_reactivo, tiempo_terminar_reactivo, palabra_del_usuario } = req.body;
        if (!usuario_id || !tipo_subtipo) {
            return res.status(400).json({ message: 'usuario_id y tipo_subtipo son requeridos.' });
        }
        const resultado = await resultadosModel.registrarResultado({ usuario_id, tipo_subtipo, tiempo_inicio_reactivo, tiempo_terminar_reactivo, palabra_del_usuario });
